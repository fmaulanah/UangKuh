...
### 3H --- Adjustment UI --- COMPLETE / LOCKED

```text
[✓] 3H.1 Adjustment Foundation
[✓] 3H.2 Create Positive Adjustment
[✓] 3H.3 Create Negative Adjustment
[✓] 3H.4 Validation & Edge Cases
[✓] 3H.5 Balance / Dashboard / History Integration
[✓] 3H.6 Final Validation
```
...
- Add Adjustment screen integrated with existing GoRouter Dashboard branch
- Adjustment is available from Dashboard Quick Actions
- active account selection reuses accountListProvider
- archived accounts are excluded
- transaction date and optional description supported
- future transaction dates are blocked
- form controllers are owned and disposed by screen State

Adjustment contract
- reuses existing TransactionRepository.createAdjustment
- selected account maps to the adjusted account / destinationAccountId
- sourceAccountId remains null
- categoryId remains null
- expenseType remains null
- amount is a signed integer
- amount != 0
- no locked schema or repository contract changes required

Positive / negative adjustment
- positive Adjustment increases selected account balance
- negative Adjustment decreases selected account balance
- negative account balances remain allowed
- signed amount is preserved without abs conversion
- Adjustment is not treated as Income or Expense

Validation / edge cases
- account selection required
- empty amount rejected
- zero amount rejected
- invalid/non-numeric amount rejected
- positive and negative amounts accepted
- optional description is normalized when empty/whitespace
- double-save protection prevents duplicate user submission
- cancel/back creates no transaction

Balance / Dashboard / History integration
- Adjustment affects selected Account balance through locked ledger formula
- Dashboard Total Balance follows signed Adjustment
- Dashboard Account Overview follows signed Adjustment
- Adjustment appears in Transaction History
- Adjustment appears in Dashboard Recent Transactions
- Monthly Income remains unchanged
- Monthly Expense remains unchanged
- Monthly Net remains unchanged
- Recurring / Plan state remains unchanged
- soft-delete reverses Adjustment financial effect
- deleted Adjustment disappears from active History / Recent
- persistence after cold restart validated

Regression / final validation
- positive Adjustment end-to-end PASS
- negative Adjustment end-to-end PASS
- delete / reverse returns balance to expected baseline
- Expense / Income / Transfer / Recurring behavior preserved
- locked V1 schema unchanged
- locked financial ledger formula unchanged
- completed 3A--3G behavior preserved
- flutter analyze PASS
- flutter test PASS
- flutter run / cold-start validation PASS
```

3H completes the local Adjustment workflow while preserving the locked
ledger contract. Adjustment remains a signed balance correction and is
not reclassified as Income or Expense.
...
Phase 3 local feature UI through Adjustment is complete.

Before implementation continues, validate the canonical roadmap for the
next approved phase/section rather than inventing a new 3I scope.

```text
NEXT = REVIEW ROADMAP AFTER 3H
```

Do not jump directly to Firebase unless the approved roadmap confirms it.
...
3. Phase 3 sections 3A through 3H are complete/locked unless a real blocker or regression is discovered.
...
Do not restart Phase 1, Phase 2, or Phase 3 sections 3A--3H.
...
[✓] Bug Fix
[✓] Refactor Session Bootstrap
[✓] Firebase Auth Integration
[✓] Register
[✓] Login
[✓] Auto Login
[✓] Session Bootstrap with Firebase UID
[✓] Drift User Bootstrap
[✓] Firebase UID ↔ SQLite User Mapping
[ ] Logout

Status:
Authentication Foundation COMPLETE / LOCKED

================================================
PHASE 5 — CLOUD SYNC
================================================

5.1 Logout & Session Cleanup
--------------------------------
[ ] Logout button
[ ] Firebase SignOut
[ ] Clear AppSession
