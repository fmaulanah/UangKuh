...
Phase 3 continues after the completed 3E Transaction History checkpoint.

```text
NEXT = 3F --- Recurring / Plan UI
```

Before starting 3F:
1. Validate each step against the locked Recurring domain rules.
2. Reuse the completed Recurring repository foundation.
3. Implement step-by-step without changing the locked V1 schema.
4. Run `flutter analyze` and relevant tests at final checkpoints.
5. Do not jump to Firebase outside the approved Phase 3 sequence.
6. Do not modify completed 3A--3E behavior unless a real blocker or regression is discovered.
...
3. Phase 3 sections 3A through 3E are complete/locked unless a real blocker or regression is discovered.
...
Do not restart Phase 1, Phase 2, or Phase 3 sections 3A--3E.
